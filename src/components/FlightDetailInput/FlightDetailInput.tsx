...
const FlightDetailInput = () => {
  ...
  useEffect(() => {
    ...
    const departureOptions = airportList.filter(
      (port) => port !== selectedDestination
    );
    const destinationOptions = airportList.filter(
      (port) => port !== selectedDeparture
    );
  ...
  return (
    <form onSubmit={handleSearch} className="w-6/12 p-2 mx-auto" action="">
      ...
      <div className="text-center pt-3">
        <button className="w-60 mt-10 mb-16 bg-indigo-800 hover:bg-indigo-900 focus:ring text-white font-bold py-4 px-8 rounded-full">
          Go!
        </button>
      </div>
    </form>
