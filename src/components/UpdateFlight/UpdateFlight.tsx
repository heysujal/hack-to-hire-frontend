...
const UpdateFlight = () => {
  ...
  const handleSearch = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.get(`${SERVER_ENDPOINT}/admin/flights/6E ${flightId}`);
  ...
  const handleUpdate = async (e) => {
    ...
    if (flightDetails) {
      try {
        const response = await axios.put(`${SERVER_ENDPOINT}/admin/flights/${flightDetails._id}`, flightDetails);
          ...
          {showEditor && flightDetails && (
            <>
              ...
              <DateTimePicker
                ...
                onChange={(date) => handleChange('scheduled_departure', date.toISOString())}
                required
              />
              <DateTimePicker
                label="Scheduled Arrival"
                value={new Date(flightDetails.scheduled_arrival)}
                onChange={(date) => handleChange('scheduled_arrival', date.toISOString())}
                minDate={new Date(flightDetails.scheduled_departure)}
                required
              />
              <DateTimePicker
                label="Actual Departure"
                value={flightDetails.actual_departure ? new Date(flightDetails.actual_departure) : null}
                onChange={(date) => handleChange('actual_departure', date.toISOString())}
              />
              <DateTimePicker
                label="Actual Arrival"
                value={flightDetails.actual_arrival ? new Date(flightDetails.actual_arrival) : null}
                onChange={(date) => handleChange('actual_arrival', date.toISOString())}
                minDate={flightDetails.actual_departure ? new Date(flightDetails.actual_departure) : new Date(flightDetails.scheduled_departure)}
              />
              <Select
                required
                label="Status"
